...
- A real, tested Flask app (178 passing tests, `ruff` clean, CI on every push).
...
- [ ] **Set `MONTHLY_PRIORITY_CALL_CEILING` to a low number** (e.g. `200`). This
      caps the expensive premium-tier calls per month on your key. The app
...
5. Deploy. Open the URL, upload a small CSV, confirm it works. Once it is live,
   put that URL in the repo description and the README.
...
- **Don't** deploy with `BILLING_ENABLED=true` and `MONTHLY_PRIORITY_CALL_CEILING=0`;
  that's an uncapped bill. (The app warns, but don't rely on the warning.)
- **Don't** expose the app with `RATE_LIMIT_ENABLED=false`.
- **Don't** run multiple workers with `memory://` rate-limit storage (caps don't hold).
- **Don't** commit `.env` or paste your `GEMINI_API_KEY` anywhere public.
- **Don't** skip the Gemini-side budget cap. It is the one control that *cannot* be
