...
logger = logging.getLogger('codecaster.config')
...
class Config:
    ...
    runner_image: str = 'codecaster-runner'
    exec_timeout: int = 60
    ...
    def from_env(cls):
        cfg = cls(
            ...
            s3_prefix=os.environ.get('S3_PREFIX', 'codecaster').strip(),
            file_ttl_seconds=_env_int('FILE_TTL_SECONDS', 7200),
            sandbox_mode=os.environ.get('SANDBOX_MODE', 'subprocess').strip().lower(),
            runner_image=os.environ.get('RUNNER_IMAGE', 'codecaster-runner').strip(),
    ...
    def resolved_upload_root(self):
        ...
        self.upload_root = tempfile.mkdtemp(prefix='codecaster_')
        return self.upload_root
    ...
    def resolved_database_url(self, instance_dir):
        ...
        db_path = os.path.join(instance_dir, 'codecaster.db')
